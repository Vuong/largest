...
export class ApiKeyStrategy extends PassportStrategy(
  ...
  async validate(apikey: string): Promise<EUser | false> {
    let apiValidation = await IsApiKey().safeParseAsync(apikey);
    if (!apiValidation.success) {
      this.logger.warn('Invalid apikey format: ' + apikey);
      return false;
    }

    let apikeyResult = await this.apikeyDB.resolve(apikey);
    if (HasFailed(apikeyResult)) {
      this.logger.warn('Invalid apikey: ' + apikey);
      return false;
    }

    let user = EUserBackend2EUser(apikeyResult.user);

    let userValidation = await EUserSchema.safeParseAsync(user);
